    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['topic:judgment:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['topic:judgment:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <!-- 列表 -->
    <el-table v-loading="loading" :data="list" type="index">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['topic:judgment:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['topic:judgment:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createJudgment, updateJudgment, deleteJudgment, getJudgment, getJudgmentPage, exportJudgmentExcel } from "@/api/topic/judgment";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        answer: [{ required: true, message: "参考答案：对/错不能为空", trigger: "blur" }],
        ...
        isWrong: [{ required: true, message: "是否错题不能为空", trigger: "change" }],
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getJudgmentPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateJudgment(this.form).then(response => {
        ...
        // 添加的提交
        createJudgment(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除判断题编号为"' + id + '"的数据项?').then(function() {
          return deleteJudgment(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有判断题数据项?').then(() => {
          this.exportLoading = true;
          return exportJudgmentExcel(params);
        }).then(response => {
          this.$download.excel(response, '判断题.xls');
