    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['topic:knowledge:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['topic:knowledge:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['topic:knowledge:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['topic:knowledge:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createKnowledge, updateKnowledge, deleteKnowledge, getKnowledge, getKnowledgePage, exportKnowledgeExcel } from "@/api/topic/knowledge";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        subjectId: [{ required: true, message: "科目标识不能为空", trigger: "change" }],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getKnowledgePage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createKnowledge(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除知识点编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有知识点数据项?').then(() => {
          this.exportLoading = true;
          return exportKnowledgeExcel(params);
        }).then(response => {
          this.$download.excel(response, '知识点.xls');
